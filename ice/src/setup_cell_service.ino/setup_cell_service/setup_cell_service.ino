...
#define DESTINATION_PHONE_NUMBER "1555444333"
...
void setup() 
{
  ...
  while(!connectionGood){
    ...
    while(Serial1.available() >0) {
      ...
      // check currentString to see if network status is "OK" which means we are connected
      if((currentString.substring(currentString.length()-2, currentString.length()) == "OK")){
      connectionGood = true;
      Serial.println(); 
      Serial.println("Modem Activation Successful!");  
      }
    }
  }
  while(1);
